...
//            qreal score=50.0+(qrand() % 50);//随机数
            qreal score= QRandomGenerator::global()->bounded(50,101);   //随机数, [50,100]
            avgScore+=score;
...
//    chart->removeAxis(chart->axisX()); //删除坐标轴，函数deprecated
...
//    chart->setAxisX(axisX, series); //设置横坐标, setAxisX()函数过时了
...
//    chart->setAxisY(axisY, series); //过时的函数用法
...
//    chart->removeAxis(chart->axisX()); //删除坐标轴,过时的函数
...
//    chart->setAxisX(axisX, series);       //过时的函数用法
    chart->addAxis(axisX, Qt::AlignBottom);
...
//    chart->removeAxis(chart->axisX());    //过时的函数用法
//    chart->removeAxis(chart->axisY());
    removeAllAxis(chart);
...
//    qsrand(QTime::currentTime().second());//随机数种子,过时的函数
...
//    chart->axisX()->setTitleText("X 轴");      //函数axisX()过时，换成下面的代码
...
//    chart->axisY()->setTitleText("Y 轴");//函数axisX()过时，换成下面的代码
