...
//    QStringList headerList=header.split(QRegExp("\\s+"),QString::SkipEmptyParts);  //QRegExp 过时了
...
//一个或多个空格、TAB等分隔符隔开的字符串， 分解为一个StringList
//        QStringList tmpList=aLineText.split(QRegExp("\\s+"),QString::SkipEmptyParts); //QRegExp 过时了
